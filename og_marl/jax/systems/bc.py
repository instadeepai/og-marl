...
GRU_LAYER_SIZE = 64
NUM_ACTS = 9
NUM_TRAIN_STEPS_PER_EPOCH = 2500
NUM_EVALS = 10
...
def evaluation(init_carry, params, environment):
    ...
    return {"mean_episode_return": sum(episode_returns)/NUM_EVALS}
...
environment = get_environment(ENV, SCENARIO)
store = FlashbaxBufferStore("Good_3m")
...
wandb.log(eval_logs)
print(f"First Eval Episode Return: {eval_logs['mean_episode_return']}")
for i in range(100):
    start_time = time.time()
    params, opt_state, logs = jax.jit(train_epoch)(rng_key, params, opt_state, buffer_state)
    end_time = time.time()
    ...
    print(f"Epoch: {i}  |   Loss: {round(float(jnp.mean(logs['loss'])), 3)} |   Eval Episode Return: {eval_logs['mean_episode_return']}")
