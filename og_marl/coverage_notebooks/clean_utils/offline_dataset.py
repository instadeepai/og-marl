...
def analyse_vault(
    ...
    if visualise:
        ...
        plt.xticks(range(len(all_uid_returns)), list(all_uid_returns.keys()),rotation=90)
        if save:
            plt.savefig(rel_dir+"/"+vault_name+"/"+"violin_"+str(vault_name)+".pdf",format='pdf',bbox_inches='tight')
