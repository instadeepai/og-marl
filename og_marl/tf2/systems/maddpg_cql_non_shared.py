...
def unroll_rnn(rnn_networks: List[snt.Module], inputs: Tensor, resets: Tensor, agent_idx: Optional[int]=None) -> Tensor:
...
class MADDPGCQLNonSharedSystem(BaseOfflineSystem):
    ...
    def _tf_train_step(self, experience: Dict[str, Any]) -> Dict[str, Numeric]:
        ...
        variables = []
        for n in range(N):
            variables = variables + [*self.policy_networks[n].trainable_variables]
...
def run_experiment(cfg: DictConfig) -> None:
    ...
    system = MADDPGCQLNonSharedSystem(env, logger, **cfg["system"])
