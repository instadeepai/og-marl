...
class FACMACCQLSystem(IDDPGSystem):
    ...
    def _tf_train_step(self, batch):
        ...
        target_qs = self._target_critic_network(
            observations, env_states, target_actions, target_actions, resets
        )
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            qs = self._critic_network(
                observations, env_states, replay_actions, replay_actions, resets
            )
            ...
            # Policy Loss
            qs = self._critic_network(
                observations, env_states, online_actions, online_actions, resets
            )
            qs = self._mixer(qs, env_states)
