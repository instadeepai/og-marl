...
class CriticNetwork(snt.Module):
    ...
    def __call__(self, observations: tf.Tensor, actions: tf.Tensor) -> tf.Tensor:
        input_tensor = tf.concat([observations, actions], axis=-1)
...
class ISACNSystem(BaseOfflineSystem):
    ...
    def _tf_train_step(self, experience: Dict[str, Any]) -> Dict[str, Numeric]:
        ...
        for target_critic in self.target_critics:
            target_q = target_critic(next_env_states, target_actions)
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            policy_qs = []
            for critic in self.critics:
                policy_qs.append(critic(env_states, online_actions))
...
@hydra.main(version_base=None, config_path="configs", config_name="isac_n")
