...
from og_marl.baselines.jax_systems.networks.utils.cont_oryx import (
    act_encoder_fn,
...
class Decoder(nn.Module):
    ...
    n_agents: int
    action_dim: int
    action_space_type: str = _CONTINUOUS
    ...
    def setup(self) -> None:
        ...
        ) # outputs q value of action taken
    ...
    def __call__(
        ...
        means = self.policy_head(x)

        # concat actions and x
        q_value = self.critic_head(jnp.concatenate([jax.lax.stop_gradient(action), x], axis=-1))

        return means, q_value, updated_hstates
...
class ContOryxNetwork(nn.Module):
    """Sable network module modified to output q values as well as actions."""
    ...
    def setup(self) -> None:
        if self.action_space_type not in [_CONTINUOUS]:
        ...
        assert self.action_space_type == _CONTINUOUS, "Only continuous actions are supported."
        self.train_decoder_fn = partial(
            continuous_train_decoder_fn,
            n_agents=self.n_agents,
            chunk_size=self.memory_config.chunk_size,
        )
        self.autoregressive_act = continuous_autoregressive_act  # type: ignore
    ...
    def __call__(
        ...
        action, log_probs, q_value, entropy = self.train_decoder_fn(
        ...
        return action, log_probs, q_value, entropy
    ...
    def get_actions(
        ...
        output_actions, log_prob, updated_dec_hs = self.autoregressive_act(
            decoder=self.decoder,
            obs_rep=obs_rep,
            legal_actions=legal_actions,
        ...
        return output_actions, log_prob, updated_hs