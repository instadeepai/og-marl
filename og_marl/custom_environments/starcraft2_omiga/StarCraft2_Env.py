...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        reward_sparse : bool, optional
            Receive 1/-1 reward for winning/losing an episode (default is
            False). When rest of reward parameters are ignored if True.
        ...
        reward_defeat : float, optional
            The reward for losing in an episode (default is 0). This value
    ...
    def _launch(self):
        ...
        self.terrain_height = (
            np.flip(
                np.transpose(
                    np.array(list(map_info.terrain_height.data)).reshape(self.map_x, self.map_y)
                ),
                1,
            )
            / 255
    ...
    def step(self, smac_actions):
        ...
        actions_int = [int(a) for a in smac_actions]
    ...
    def reward_battle(self):
        ...
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                # did not die so far
                prev_health = (
                    self.previous_ally_units[al_id].health + self.previous_ally_units[al_id].shield
                )
                if al_unit.health == 0:
                    # just died
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                prev_health = (
                    self.previous_enemy_units[e_id].health + self.previous_enemy_units[e_id].shield
                )
                if e_unit.health == 0:
    ...
    def get_obs_agent(self, agent_id):
        ...
        agent_obs = np.concatenate(
            (ally_feats.flatten(), enemy_feats.flatten(), move_feats.flatten(), own_feats.flatten())
        )
    ...
    def only_medivac_left(self, ally):
        ...
        if ally:
            units_alive = [
                a for a in self.agents.values() if (a.health > 0 and a.unit_type != self.medivac_id)
            ]
