...
flags.DEFINE_string(
    "dataset", "Poor", "Dataset type.: 'Good', 'Medium', 'Poor' or 'Replay' "
)  # Usually just use the good data, except in an ablation study over data types
flags.DEFINE_string(
    "system", "qmix+cql", "System name."
)  # NOTE: just keep on qmix+cql, even for online QMIX, experiment with IDRQN+CQL
flags.DEFINE_integer("seed", 42, "Seed.")

# Offline params to experiment with
flags.DEFINE_float(
    "offline_training_steps", 1000, "Number of offline training steps."
)  # 1000, 2000, 3000, 4000, 5000
flags.DEFINE_float(
    "offline_cql_weight", 2, "CQL Weight during offline training."
)  # NOTE: no need to change this

# Online params to experiment with
flags.DEFINE_string(
    "online_buffer", "mixed", "Set the online buffer to be 'mixed' or 'online'."
)  # online, mixed
flags.DEFINE_float("online_cql_weight", 2, "CQL Weight during online training.")  # 0, 1, 2

# NOTE: Keep fixed for now
flags.DEFINE_float(
    "eps_decay_steps", 1000, "Timesteps during which to decay epsilon for online training."
)  # Probably just keep fixed for online and pre training exps

flags.DEFINE_string(
    "suffix", "", "Additional information to be contained in the WandB project name."
)
...
def main(_):
    ...
    project_name = (
        ...
        + FLAGS.system
        + "_"
        + str(FLAGS.offline_training_steps)
        + "_"
        + FLAGS.scenario
        + "_"
        + FLAGS.dataset
    ...
    offline_buffer = FlashbaxReplayBuffer(
        sequence_length=10, sample_period=1, batch_size=64, rewards_to_go=r2go, seed=FLAGS.seed
    )

    download_and_unzip_vault(FLAGS.env, FLAGS.scenario)

    is_vault_loaded = offline_buffer.populate_from_vault(
        FLAGS.env, FLAGS.scenario, FLAGS.dataset, discount=0.99
    )
    ...
    system.train_offline(
        offline_buffer,
        max_trainer_steps=FLAGS.offline_training_steps,
        evaluate_every=100,
        num_eval_episodes=4,
    )
    ...
    online_buffer = FlashbaxReplayBuffer(
        ...
        rewards_to_go=r2go,
        seed=FLAGS.seed,
    )  # NOTE: keep max_size fixed

    if FLAGS.online_buffer == "mixed":
        online_buffer = MixedBuffer(
            online_buffer, offline_buffer
        )  # setup mixed buffer 50/50 online/offline batches
    ...
    system.train_online(
        online_buffer, max_env_steps=100000, train_period=20
    )  # NOTE: keep environment steps fixed and train period
